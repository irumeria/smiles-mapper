...
def smiles_to_map(smiles):
    ...
    def piBondConveter(nodeMap):
        for node in nodeMap:
            for neighbor in node['neighbors']:
                neighbor['atom'] = nodeMap[int(neighbor['index'])]['atom'] # 动点手脚，顺便加个atom属性
    ...
    def analyse(smiles, i=0, index=0, nodeMap=[], preIndex=-1, historyBox=None):
        ...
        if re.match(r"[1-9]", smChar) and len(historyBox["loops"]) >= int(smChar):
            ...
            nodeMap[preIndex]['neighbors'].append({"index":loopHeader,"bond":historyBox['bond'][0]})
            nodeMap[loopHeader]['neighbors'].append({"index":preIndex,"bond":historyBox['bond'][0]})
            ...
            nodeMap[preIndex]['neighbors'].append({"index":index,"bond":historyBox['bond'][0]})
        elif re.match(r"[\]]", smChar):
